...
const Fragen2Plugin = {
    ...
    _renderList() {
        ...
        if (filtered.length === 0) {
            ...
            filtered.forEach(function(q) {
                ...
                html += (self._editingId === q.questionId) ? self._renderEditForm(q) : self._renderCard(q);
            });
            html += '</div>';
        }
    ...
    _renderCard(q) {
        ...
        var isExpanded = this._expandedCards.has(q.questionId);
        ...
        h += '<input type="checkbox" ' + (isActive ? 'checked' : '') + ' onchange="Fragen2Plugin._toggleActive(\'' + q.questionId + '\')" style="width:16px;height:16px;cursor:pointer;accent-color:var(--accent);">';
        ...
        if (this._viewMode === 'compact') {
            h += '<button class="btn btn-small btn-secondary" onclick="Fragen2Plugin._toggleExpand(\'' + q.questionId + '\')" style="font-size:0.78rem;padding:4px 10px;">' + (isExpanded ? '▲ Weniger' : '▼ Details') + '</button>';
        }
        h += '<button class="btn btn-small btn-secondary" onclick="Fragen2Plugin._showQuestionStats(\'' + q.questionId + '\')" style="font-size:0.78rem;padding:4px 10px;">📊</button>';
        h += '<button class="btn btn-small btn-secondary" onclick="Fragen2Plugin._preview(\'' + q.questionId + '\')" style="font-size:0.78rem;padding:4px 10px;">👁️</button>';
        h += '<button class="btn btn-small" onclick="Fragen2Plugin._startEdit(\'' + q.questionId + '\')" style="font-size:0.78rem;padding:4px 10px;background:linear-gradient(135deg,#2980b9,#3498db);">✏️ Bearbeiten</button>';
        h += '<button class="btn btn-small btn-danger" onclick="Fragen2Plugin._delete(\'' + q.questionId + '\')" style="font-size:0.78rem;padding:4px 10px;">🗑️</button>';
        h += '</div>';
    ...
    _startEdit(qId) {
        ...
        var q = questions.find(function(qq) { return qq.questionId === qId; });
    ...
    _renderEditForm(q) {
        ...
        var h = '<div class="f2-card" id="f2Edit_' + qid + '" style="border-color:var(--accent);background:rgba(247,184,1,0.05);">';
        h += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">';
        h += '<strong style="color:var(--accent);">✏️ Frage ' + (qid || '?') + ' bearbeiten</strong>';
        ...
        h += '<textarea id="f2EditText_' + qid + '" rows="3" style="width:100%;padding:10px;background:rgba(255,255,255,0.1);border:2px solid rgba(255,255,255,0.2);border-radius:8px;color:var(--text);font-size:0.95rem;resize:vertical;box-sizing:border-box;">' + sanitizeHTML(q.text || '') + '</textarea>';
        ...
        h += '<select id="f2EditGroup_' + qid + '" onchange="var inp=document.getElementById(\'f2EditGroupInput_' + qid + '\');if(this.value===\'__new__\'){inp.style.display=\'block\';inp.focus();}else{inp.style.display=\'none\';}" style="flex:1;padding:6px 10px;background:rgba(255,255,255,0.1);border:2px solid rgba(255,255,255,0.2);border-radius:8px;color:var(--text);">';
        groups.forEach(function(g) {
        ...
        h += '</select>';
        h += '<input type="text" id="f2EditGroupInput_' + qid + '" placeholder="Neuer Gruppenname..." style="display:none;flex-basis:100%;padding:6px 10px;background:rgba(255,255,255,0.1);border:2px solid rgba(255,255,255,0.2);border-radius:8px;color:var(--text);font-size:0.85rem;margin-top:4px;">';
        h += '</div>';
        ...
        h += '<input type="text" id="f2EditId_' + qid + '" value="' + sanitizeHTML(qid || '') + '" style="flex:1;padding:4px 8px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:6px;color:var(--text);font-size:0.78rem;font-family:monospace;" placeholder="prefix_00001">';
        h += '</div>';
        ...
        if (qType === 'multiple-choice') {
            ...
            (q.answers || []).forEach(function(a, i) {
                ...
                h += '<input type="text" id="f2EditAns_' + qid + '_' + i + '" value="' + sanitizeHTML(a.text || '') + '" style="flex:1;padding:8px;background:rgba(255,255,255,0.1);border:2px solid ' + (a.correct ? 'var(--correct)' : 'rgba(255,255,255,0.2)') + ';border-radius:8px;color:var(--text);font-size:0.9rem;">';
                h += '<label style="display:flex;align-items:center;gap:4px;font-size:0.82rem;cursor:pointer;">';
                h += '<input type="checkbox" id="f2EditCorr_' + qid + '_' + i + '" ' + (a.correct ? 'checked' : '') + ' style="accent-color:var(--correct);"> ✓</label>';
                h += '</div>';
            });
        } else if (qType === 'text') {
            var ta = typeof getCorrectTextAnswers === 'function' ? getCorrectTextAnswers(q) : (q.correctAnswer || []);
            ...
            h += '<textarea id="f2EditTextAns_' + qid + '" rows="3" style="width:100%;padding:10px;background:rgba(255,255,255,0.1);border:2px solid rgba(255,255,255,0.2);border-radius:8px;color:var(--text);font-size:0.9rem;box-sizing:border-box;">' + sanitizeHTML(ansStr) + '</textarea>';
        } else if (qType === 'imagemap') {
            h += this._renderImEditor('edit_' + qid, q);
        }
        ...
        if (qType === 'multiple-choice' || qType === 'text') {
            ...
            h += '<input type="text" id="f2EditMedia_' + qid + '" value="' + sanitizeHTML(existingPath) + '" placeholder="z.B. medien/Bundesländer/bayern.jpg" oninput="Fragen2Plugin._liveMediaPreview(\'f2EditMedia_' + qid + '\',\'f2EditMediaPrev_' + qid + '\')" style="flex:1;padding:8px;background:rgba(255,255,255,0.1);border:2px solid rgba(255,255,255,0.2);border-radius:8px;color:var(--text);font-size:0.85rem;box-sizing:border-box;">';
            h += '<input type="file" id="f2EditMediaBrowse_' + qid + '" accept="image/*" style="display:none;" onchange="Fragen2Plugin._handleMediaBrowse(event,\'f2EditMedia_' + qid + '\')">';
            h += '<button type="button" class="btn btn-secondary btn-small" onclick="document.getElementById(\'f2EditMediaBrowse_' + qid + '\').click()" style="white-space:nowrap;padding:6px 12px;font-size:0.8rem;">📂 Durchsuchen</button>';
            if (hasMediaData) {
                h += '<button type="button" class="btn btn-secondary btn-small" onclick="document.getElementById(\'f2EditMedia_' + qid + '\').value=\'\';Fragen2Plugin._editMediaCleared=\'' + qid + '\';document.getElementById(\'f2EditMediaPrev_' + qid + '\').innerHTML=\'\';" style="padding:6px 8px;font-size:0.8rem;" title="Bild entfernen">✕</button>';
            }
            h += '</div>';
            ...
            h += '<div id="f2EditMediaPrev_' + qid + '" style="margin-top:6px;">';
            if (q.media) {
        ...
        h += '<input type="text" id="f2EditExpl_' + qid + '" value="' + sanitizeHTML(q.explanation || '') + '" placeholder="Wird nach falscher Antwort angezeigt..." style="width:100%;padding:8px;background:rgba(255,255,255,0.08);border:2px solid rgba(255,255,255,0.15);border-radius:8px;color:var(--text);font-size:0.85rem;box-sizing:border-box;">';
        ...
        h += '<input type="text" id="f2EditExplMedia_' + qid + '" value="' + sanitizeHTML(explMediaPath) + '" placeholder="Bild zur Erklärung (z.B. medien/erkl_001.jpg)" oninput="Fragen2Plugin._liveMediaPreview(\'f2EditExplMedia_' + qid + '\',\'f2EditExplMediaPrev_' + qid + '\')" style="flex:1;padding:6px 8px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:6px;color:var(--text);font-size:0.8rem;box-sizing:border-box;">';
        h += '<input type="file" id="f2EditExplMediaBrowse_' + qid + '" accept="image/*" style="display:none;" onchange="Fragen2Plugin._handleMediaBrowse(event,\'f2EditExplMedia_' + qid + '\')">';
        h += '<button type="button" class="btn btn-secondary btn-small" onclick="document.getElementById(\'f2EditExplMediaBrowse_' + qid + '\').click()" style="padding:4px 8px;font-size:0.75rem;">📂</button>';
        if (explMediaPath) h += '<button type="button" class="btn btn-secondary btn-small" onclick="document.getElementById(\'f2EditExplMedia_' + qid + '\').value=\'\';document.getElementById(\'f2EditExplMediaPrev_' + qid + '\').innerHTML=\'\';" style="padding:4px 6px;font-size:0.75rem;" title="Bild entfernen">✕</button>';
        h += '</div>';
        h += '<div id="f2EditExplMediaPrev_' + qid + '" style="margin-top:4px;">';
        ...
        h += '<input type="text" id="f2EditHint_' + qid + '" value="' + sanitizeHTML(q.hint || '') + '" placeholder="Wird bei Hinweis-Fähigkeit angezeigt..." style="width:100%;padding:8px;background:rgba(255,255,255,0.08);border:2px solid rgba(255,255,255,0.15);border-radius:8px;color:var(--text);font-size:0.85rem;box-sizing:border-box;">';
        var hintMediaPath = (q.hintMedia && q.hintMedia.path) ? q.hintMedia.path : '';
        h += '<div style="display:flex;gap:8px;align-items:center;margin-top:4px;">';
        h += '<input type="text" id="f2EditHintMedia_' + qid + '" value="' + sanitizeHTML(hintMediaPath) + '" placeholder="Bild zum Hinweis (z.B. medien/hint_001.jpg)" oninput="Fragen2Plugin._liveMediaPreview(\'f2EditHintMedia_' + qid + '\',\'f2EditHintMediaPrev_' + qid + '\')" style="flex:1;padding:6px 8px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:6px;color:var(--text);font-size:0.8rem;box-sizing:border-box;">';
        h += '<input type="file" id="f2EditHintMediaBrowse_' + qid + '" accept="image/*" style="display:none;" onchange="Fragen2Plugin._handleMediaBrowse(event,\'f2EditHintMedia_' + qid + '\')">';
        h += '<button type="button" class="btn btn-secondary btn-small" onclick="document.getElementById(\'f2EditHintMediaBrowse_' + qid + '\').click()" style="padding:4px 8px;font-size:0.75rem;">📂</button>';
        if (hintMediaPath) h += '<button type="button" class="btn btn-secondary btn-small" onclick="document.getElementById(\'f2EditHintMedia_' + qid + '\').value=\'\';document.getElementById(\'f2EditHintMediaPrev_' + qid + '\').innerHTML=\'\';" style="padding:4px 6px;font-size:0.75rem;" title="Bild entfernen">✕</button>';
        h += '</div>';
        h += '<div id="f2EditHintMediaPrev_' + qid + '" style="margin-top:4px;">';
        ...
        h += '<button class="btn" onclick="Fragen2Plugin._saveEdit(\'' + qid + '\')" style="padding:10px 24px;">💾 Speichern</button>';
    ...
    _showQuestionStats(qId) {
        var q = questions.find(function(qq) { return qq.questionId === qId; });
        if (!q) return;
        var qid = q.questionId || '???';

        // Aggregate across all users
        var totalAsked = 0, totalCorrect = 0;
        var userRows = [];
        users.forEach(function(u) {
            var s = u.questionStats && u.questionStats[qid];
            if (s && s.asked > 0) {
                totalAsked += s.asked || 0;
                totalCorrect += s.correct || 0;
                userRows.push({
                    name: u.name,
                    asked: s.asked || 0,
                    correct: s.correct || 0,
                    pct: Math.round(((s.correct || 0) / s.asked) * 100),
                    streak: s.consecutiveCorrect || 0,
                    lastAsked: s.lastAsked ? new Date(s.lastAsked).toLocaleDateString('de-DE') : '–'
                });
            }
        });
        var totalPct = totalAsked > 0 ? Math.round((totalCorrect / totalAsked) * 100) : -1;
        var pctColor = totalPct >= 75 ? '#2ecc71' : totalPct >= 50 ? '#f1c40f' : totalPct >= 0 ? '#e74c3c' : 'rgba(255,255,255,0.4)';

        var h = '<div class="f2-preview-overlay" onclick="if(event.target===this)Fragen2Plugin._closeQuestionStats()">';
        h += '<div class="f2-preview-card" style="max-width:700px;">';
        h += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">';
        h += '<span style="font-size:0.8rem;opacity:0.5;">📊 Fragen-Statistik</span>';
        h += '<button class="btn btn-small btn-secondary" onclick="Fragen2Plugin._closeQuestionStats()">✕</button>';
        h += '</div>';

        var qText = q.text || '';
        h += '<div style="font-size:0.95rem;font-weight:600;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,0.1);">' + sanitizeHTML(qText.length > 120 ? qText.substring(0, 120) + '…' : qText) + '</div>';

        // 3 overview tiles
        h += '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:20px;">';
        h += '<div style="background:rgba(255,255,255,0.05);border-radius:10px;padding:14px;text-align:center;">';
        h += '<div style="font-size:1.8rem;font-weight:700;color:' + pctColor + ';">' + (totalPct >= 0 ? totalPct + '%' : '–') + '</div>';
        h += '<div style="font-size:0.8rem;opacity:0.6;margin-top:4px;">Erfolgsrate gesamt</div></div>';
        h += '<div style="background:rgba(255,255,255,0.05);border-radius:10px;padding:14px;text-align:center;">';
        h += '<div style="font-size:1.8rem;font-weight:700;">' + totalAsked + '</div>';
        h += '<div style="font-size:0.8rem;opacity:0.6;margin-top:4px;">Mal gestellt</div></div>';
        h += '<div style="background:rgba(255,255,255,0.05);border-radius:10px;padding:14px;text-align:center;">';
        h += '<div style="font-size:1.8rem;font-weight:700;color:#2ecc71;">' + totalCorrect + '</div>';
        h += '<div style="font-size:0.8rem;opacity:0.6;margin-top:4px;">Richtig beantwortet</div></div>';
        h += '</div>';

        // Per-user table
        if (userRows.length === 0) {
            h += '<div style="text-align:center;padding:30px;opacity:0.5;">Noch keine Daten – Frage wurde noch nicht gestellt.</div>';
        } else {
            h += '<div style="font-weight:600;margin-bottom:10px;font-size:0.9rem;">Pro Nutzer</div>';
            h += '<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;font-size:0.87rem;">';
            h += '<thead><tr style="border-bottom:1px solid rgba(255,255,255,0.15);">';
            h += '<th style="padding:8px;text-align:left;">Nutzer</th>';
            h += '<th style="padding:8px;text-align:center;">Gestellt</th>';
            h += '<th style="padding:8px;text-align:center;">Richtig</th>';
            h += '<th style="padding:8px;text-align:center;">Rate</th>';
            h += '<th style="padding:8px;text-align:center;">Streak</th>';
            h += '<th style="padding:8px;text-align:center;">Zuletzt</th>';
            h += '</tr></thead><tbody>';
            userRows.forEach(function(r) {
                var rColor = r.pct >= 75 ? '#2ecc71' : r.pct >= 50 ? '#f1c40f' : '#e74c3c';
                h += '<tr style="border-bottom:1px solid rgba(255,255,255,0.06);">';
                h += '<td style="padding:8px;font-weight:600;">' + sanitizeHTML(r.name) + '</td>';
                h += '<td style="padding:8px;text-align:center;">' + r.asked + 'x</td>';
                h += '<td style="padding:8px;text-align:center;">' + r.correct + '</td>';
                h += '<td style="padding:8px;text-align:center;"><span style="color:' + rColor + ';font-weight:700;">' + r.pct + '%</span></td>';
                h += '<td style="padding:8px;text-align:center;">' + (r.streak > 0 ? '🔥 ' + r.streak : '–') + '</td>';
                h += '<td style="padding:8px;text-align:center;opacity:0.65;">' + r.lastAsked + '</td>';
                h += '</tr>';
            });
            h += '</tbody></table></div>';

            // Trend hint
            var improving = userRows.filter(function(r) { return r.streak >= 3; }).length;
            if (improving > 0) {
                h += '<div style="margin-top:14px;padding:10px 14px;background:rgba(46,204,113,0.1);border:1px solid rgba(46,204,113,0.3);border-radius:8px;font-size:0.85rem;color:#2ecc71;">🔥 '
                    + improving + (improving === 1 ? ' Nutzer hat' : ' Nutzer haben')
                    + ' einen aktuellen Richtig-Streak von ≥ 3 – Tendenz steigend!</div>';
            }
        }

        h += '</div></div>';
        var overlay = document.createElement('div');
        overlay.id = 'f2StatsOverlay';
        overlay.innerHTML = h;
        document.body.appendChild(overlay);
    },

    _closeQuestionStats() {
        var el = document.getElementById('f2StatsOverlay');
        if (el) el.remove();
    },

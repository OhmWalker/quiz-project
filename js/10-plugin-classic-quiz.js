...
const ClassicQuizPlugin = {
    ...
    _processAnswer(isCorrect, details) {
        ...
        if (currentUser) {
            ...
            const _mo = qs.lastAsked.slice(0, 7);
            if (!qs.monthlyStats) qs.monthlyStats = {};
            if (!qs.monthlyStats[_mo]) qs.monthlyStats[_mo] = { asked: 0, correct: 0 };
            qs.monthlyStats[_mo].asked++;
            if (isCorrect) {
                qs.correct++;
                qs.monthlyStats[_mo].correct++;

...
function _fqRenderList(container) {
    ...
    const rows = filtered.map(q => `
        <tr>
            ...
            <td style="max-width:420px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">${_fqEsc(q.text)}</td>
    ...
    container.innerHTML = `
        ...
        <div class="card" style="padding:0;overflow:hidden">
            <table class="info-table" style="font-size:0.88rem;margin:0">
                ...
                <tbody>
                    ${rows || '<tr><td colspan="5" style="text-align:center;padding:30px;opacity:0.4">Keine Fragen in dieser Gruppe</td></tr>'}
                </tbody>
            </table>
        </div>`;
}
...
function _fqRenderForm(container) {
    ...
    const groupOptions = groups.map(g =>
        `<option value="${_fqEsc(g)}">${_fqEsc(g)}</option>`
    ).join('');
    ...
    const mediaSection = isIm ? '' : `
            ...
            <details style="margin-top:12px">
                ...
                <input type="text" id="fqMedia" placeholder="Pfad zur Bilddatei, z.B. bilder/frage01.jpg"
    ...
    container.innerHTML = `
        <div class="card">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
                ...
                <h2 style="margin:0;font-size:1.2rem">Neue Frage</h2>
            </div>
            ...
            <details style="margin-top:8px">
                ...
                <textarea id="fqExpl" rows="2" placeholder="Erklärungstext nach der Antwort"
                    style="margin:8px 0 4px;resize:vertical;font-size:0.9rem"></textarea>
                <input type="text" id="fqExplMedia" placeholder="Pfad zum Erklärungs-Bild"
            ...
            <details style="margin-top:8px">
                ...
                <textarea id="fqHint" rows="2" placeholder="Hinweistext für Hint-Fähigkeit"
                    style="margin:8px 0 4px;resize:vertical;font-size:0.9rem"></textarea>
                <input type="text" id="fqHintMedia" placeholder="Pfad zum Hinweis-Bild"
                    style="margin:0;font-size:0.9rem">
            </details>

            <div style="display:flex;gap:10px;margin-top:24px">
                <button class="btn btn-small" onclick="_fqSaveNew()">
                    💾 Frage speichern
                </button>
                <button class="btn btn-small btn-secondary" onclick="_fqCloseForm()"
                    style="background:var(--overlay-10);box-shadow:none">
...
function _fqSaveNew() {
    ...
    // Nächste Display-Nummer + stabile ID
    ...
    const newQ = {
        ...
        active:          true,
        _fileGroup:      group,
        media:           imMedia || (mediaPath ? { type: 'image', path: mediaPath } : null),
